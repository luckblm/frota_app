...
muni_Server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    #Download
    downcat <- reactive({
      frota %>% filter(local == input$local, ano == input$anolocal, variavel == "Categoria do veículo") %>% 
    ...
    #Download
    downcor <- reactive({
      frota %>% filter(local == input$local, ano == input$anolocal, variavel == "Cor do veículo") %>% 
        arrange(valor) %>% select(ri,local,variavel,categoria,ano,valor)
    })
    ...
    #Download
    downcombu <- reactive({
      frota %>% filter(local == input$local, ano == input$anolocal, variavel == "Tipo de combustível(s) utilizado(s)") %>% 
        arrange(valor) %>% select(ri,local,variavel,categoria,ano,valor)
    })
    ...
    #Download
    downesp <- reactive({
      frota %>% filter(local == input$local, ano == input$anolocal, variavel == "Espécie de Veículo") %>% 
        arrange(valor) %>% select(ri,local,variavel,categoria,ano,valor) 
    })
    ...
    dowtab <- reactive({
      categoria <-
        frota %>% 
        filter(local == input$local, variavel == "Categoria do veículo", ano == input$anolocal) %>% 
        mutate(Percentual = (valor / sum(valor, na.rm = TRUE)) * 100) %>%  
        arrange(desc(valor)) %>%  slice_head(n = 1)
      
      cor <-
        frota %>% 
        filter(local == input$local, variavel == "Cor do veículo", ano == input$anolocal) %>%
        mutate(Percentual = (valor / sum(valor, na.rm = TRUE)) * 100) %>%   
        arrange(desc(valor)) %>% slice_head(n = 1)
      
      combustivel <-
        frota %>% 
        filter(local == input$local,variavel == "Tipo de combustível(s) utilizado(s)", ano == input$anolocal) %>% 
        mutate(Percentual = (valor / sum(valor, na.rm = TRUE)) * 100) %>% 
        arrange(desc(valor)) %>%  slice_head(n = 1)
      
      especie <-
        frota %>% 
        filter(local == input$local, variavel == "Espécie de Veículo", ano == input$anolocal) %>% 
        mutate(Percentual = (valor / sum(valor, na.rm = TRUE)) * 100) %>%  
        arrange(desc(valor)) %>%  slice_head(n = 1)
      
      nacionalidade <-
        frota %>% 
        filter(local == input$local, variavel == "Nacionalidade do Veículo", ano == input$anolocal) %>% 
        mutate(Percentual = (valor / sum(valor, na.rm = TRUE)) * 100) %>%  
        arrange(desc(valor)) %>%  slice_head(n = 1)
